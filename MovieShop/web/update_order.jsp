<%-- 
    ...
    Created on : 31/05/2019, 8:53:52 AM
    Author     : yantoyanto
--%>
...
<html>
    ...
    <body>
     ...
     <form action="./update_order.jsp" method="POST">
                          <table class="table">
                <tr><td>User:</td><td><%= order.getUserID()%></td></tr>
                <tr><td>Movie Title:</td><td><input type="text" value="<%= order.getMovieID()%>" name="movieID"></td></tr>
                </table>
                ...
                <tr><td><input type="hidden" value="updated" name="updated"></td>   
                       ...
                       &nbsp; 
            <button class="btn btn-light" type="button" onclick="location.href = 'index.html'" > Back to Home</button>
                              </div>
                         </div>
                   </form>
            ...
            if (request.getParameter("updated") != null) {
                manager.movieTitle(order.getUserID(), movieID);
                Order orderEdit = manager.findOrder(order.getUserID());

                if (orderEdit != null) {
                    session.setAttribute("order", orderEdit);
                %>
                    <p class="outline"> <%=order.getMovieID()%> is changed to <%=movieID%></p>
                    <p style="color: red">Please ignore the text view above and check the changes on this <a href="searchorder.jsp">link</a>
                <%} else {%>
